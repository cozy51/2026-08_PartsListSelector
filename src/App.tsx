...
function App() {
  ...
  const loadFromDrive = async () => { if (!confirm('Google Driveのデータでブラウザ内のマスターを置き換えます。よろしいですか？')) return; setDriveBusy(true); try { const remote = await loadMasterFromGoogleDrive(); if (!Array.isArray(remote.specifications)||!Array.isArray(remote.units)||!Array.isArray(remote.rules)||remote.specifications.some((specification)=>!Number.isInteger(specification.no))) throw new Error('Google Driveのファイルは仕様No.方式のデータではありません。'); await updateData(remote); setNotice('Google Driveからマスターデータを読み込みました。'); } catch (error) { setDriveConnected(isGoogleDriveAuthorized()); setNotice(error instanceof Error ? error.message : 'Google Driveからの読み込みに失敗しました。'); } finally { setDriveBusy(false); } };
  ...
  return <div className="app">
    ...
    {tab==='select' ? <section className="workspace">
      ...
      {workTab==='specifications' ? <section className="panel tab-panel"><div className="section-heading"><div><span className="eyebrow">SPECIFICATIONS</span><h2>仕様選択</h2><p>各項目を選択してください。未選択の項目は空欄のまま選定できます。</p></div><span className="counter">{Object.values(selection).filter(Boolean).length} / {specs.length} 項目選択</span></div>
        <div className="spec-scroll"><div className="spec-grid">{specs.map((spec)=><label className="spec-row" key={spec.no}><span className="number">No.{String(spec.no).padStart(2,'0')}</span><span className="spec-info"><b>{spec.name}</b><small>{spec.note}</small></span><select value={selection[String(spec.no)]??''} onChange={(e)=>setSelection({...selection,[String(spec.no)]:e.target.value})}><option value="">未選択</option>{spec.options.map(o=><option key={o}>{o}</option>)}</select></label>)}</div></div>
        ...
        <div className="table-wrap"><table><thead><tr><th>No.</th><th>ユニット名</th><th>選定されたPL品番</th><th>判定状態</th><th>備考</th></tr></thead><tbody>{filtered.map(r=><tr key={r.unit.no} className={r.status} onClick={()=>setActive(r)}><td>{r.unit.no}</td><td><button className="link">{r.unit.name}</button></td><td>{r.confirmedPartNumber ?? (r.status==='selected'?r.candidates[0].partNumber:r.status==='multiple'?r.candidates.map(c=>c.partNumber).join(' / '):'—')}</td><td><span className={`badge ${r.status}`}>{r.status==='selected'?'✓':'⚠'} {statusLabel[r.status]}</span>{r.missingSpecificationNos.length>0&&<small className="missing">不足: {r.missingSpecificationNos.map(no=>`No.${no} ${specs.find(s=>String(s.no)===no)?.name??''}`).join('、')}</small>}</td><td>{r.unit.note}</td></tr>)}</tbody></table></div>
      </section> : null}
...
function MasterPanel({data,driveClientId,driveConnected,driveBusy,onDriveClientIdChange,onDriveConnect,onDriveSave,onDriveLoad,onDriveDisconnect,onReset,onExport,onImport}:{data:MasterData;driveClientId:string;driveConnected:boolean;driveBusy:boolean;onDriveClientIdChange:(value:string)=>void;onDriveConnect:()=>void;onDriveSave:()=>void;onDriveLoad:()=>void;onDriveDisconnect:()=>void;onReset:()=>void;onExport:(k:MasterKind)=>void;onImport:(k:MasterKind)=>void}) {
  ...
  const blocks: { key: MasterKind; title: string; description: string; count: number }[] = [
    { key: 'specifications', title: '仕様項目マスター', description: '仕様No.・項目名・選択肢・表示順・備考', count: data.specifications.length },
    { key: 'units', title: 'ユニットマスター', description: 'ユニットNo.・名称・表示順・備考', count: data.units.length },
    { key: 'rules', title: 'PL選定条件マスター', description: 'PL品番・名称・仕様No.ごとの適合条件', count: data.rules.length },
  ];
  const query = masterSearch.trim().toLowerCase();
  const specifications = data.specifications.filter((item) => `${item.no} ${item.name} ${item.options.join(' ')} ${item.note}`.toLowerCase().includes(query));
  ...
  return <section className="panel master-panel">
    ...
    <section className="master-content" aria-live="polite">
      ...
      <div className="master-table-wrap">
        {selectedMaster==='specifications' && <table><thead><tr><th>仕様No.</th><th>表示順</th><th>仕様項目名</th><th>選択肢</th><th>備考</th></tr></thead><tbody>{specifications.map((item)=><tr key={item.no}><td><code>{item.no}</code></td><td>{item.order}</td><td><b>{item.name}</b></td><td><div className="option-list">{item.options.map((option)=><span key={option}>{option}</span>)}</div></td><td>{item.note||'—'}</td></tr>)}</tbody></table>}
        ...
        {selectedMaster==='rules' && <table><thead><tr><th>ユニットNo.</th><th>PL品番</th><th>PL名称</th><th>適合条件（仕様No.）</th><th>備考</th></tr></thead><tbody>{rules.map((item)=><tr key={item.id}><td><code>{item.unitNo}</code></td><td><b>{item.partNumber}</b></td><td>{item.name}</td><td><div className="condition-list">{Object.entries(item.conditions).filter(([,value])=>value).map(([no,value])=><span key={no}><small>No.{no} {data.specifications.find((spec)=>String(spec.no)===no)?.name??''}</small>{value}</span>)}</div></td><td>{item.note||'—'}</td></tr>)}</tbody></table>}
