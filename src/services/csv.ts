...
export function exportMaster(data: MasterData, kind: MasterKind): string {
  ...
  const specificationCodes = data.specifications.map((x) => x.code); return toCsv([['ユニットNo.','PL品番','備考','選定対象',...specificationCodes], ...data.rules.map((x) => [x.unitNo,x.partNumber,x.note,x.selectable?'1':'0',...specificationCodes.map((code) => (x.conditions[code]??[]).join('|'))])]);
}
export function importMaster(text: string, kind: MasterKind): Specification[] | Unit[] | PLRule[] {
  ...
  const imported = rows.map((r, i) => ({ id:`csv-${Date.now()}-${i}`, unitNo:r[0], partNumber:r[1], note:r[2]??'', selectable:r[3]!=='0', conditions:Object.fromEntries(header.slice(4).map((code,j) => [code,(r[j+4]??'').split('|').filter(Boolean)]).filter(([,values]) => values.length>0)) }));
