...
describe('sample master data', () => {
  ...
  it('HOIST BASE UNIT(350X)の40行を5件のユニークPLに統合する', () => {
    const hoistRules = sampleData.rules.filter((rule) => rule.unitNo === '210');
    expect(hoistRules.map((rule) => rule.partNumber)).toEqual(['HH12100010','HH12100110','HH12100210','HH32109R10','* NO USE']);
    expect(hoistRules.find((rule) => rule.partNumber === 'HH12100010')?.conditions).toMatchObject({S051:['S051-2'],S025:['S025-1']});
    expect(hoistRules.find((rule) => rule.partNumber === 'HH32109R10')?.conditions).toMatchObject({S001:['S001-1','S001-2','S001-3','S001-8','S001-9','S001-10'],S051:['S051-3'],S025:['S025-2','S025-3']});
    expect(hoistRules.find((rule) => rule.partNumber === '* NO USE')?.selectable).toBe(false);
  });
});
